...
shards.on("shardCreate", (shard) => {
  console.log(
    bgGreenBright(
      `🔔 Shards activos, conectado en el shard ${bold(
        shard.id
      )} con un total de ${bold(shard.manager.totalShards)} shard/s activo/s.`
    )
  );
  ...
  shard.on("spawn", (shardInfo) => {
    console.log(
      bgGreenBright(
        `📢 Se ha habilitado un nuevo shard con el numero de proceso ${bold(
          shardInfo.pid
        )}`
      )
    );
